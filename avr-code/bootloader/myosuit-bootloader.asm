...
; Modified 2014 by Keenan Tims
;   -- added TX-enable output
;   -- some ugly hacks to make it compile in AVRA
;   -- docs say ATtiny841 doesn't include JMP but original code
;      used it anyway claiming flashsize > 16kb when it is 8kb
;
...
;-----------------------------------------------------------------------
;
; Important Note: B0/B1 are the defaults for making database template
;
.equ    RXPORT  = PORTB
.equ    RXPIN   = PINB
.equ    RXDDR   = DDRB
...
.def    checkl  = r22
.def    checkh  = r23                   ; checksum counter
...
.endif          ; from the IF-part on TSB-INSTALLER = 1
...
;
; xl/xh is expected to be 0 at the beginning (from routine above)
...
; Detect request for and execute Emergency Erase.
...
chpwee:
        rcall RequestConfirmation       ; request confirm
...
WaitRX:
        ...
        sbi TXPORT, TXBIT               ; and TX is actively driven "1"
		  sbi ENDDR, ENBIT
		  sbi ENPORT, ENBIT               ; run the transmitter the whole time
WRXto:
        adiw xl, 1                      ; allow port levels to set
        brne WRXto                      ; await some milliseconds
